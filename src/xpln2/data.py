...
class DATA:
  ...
  def betters(self, n):
    ...
    # tmp = sorted(self.rows, key=lambda row:self.better(self.rows[self.rows.index(row)-1], row))
  ...
  def xpln(self, best, rest):
    ...
    # reverser=True ??
    rule, most = utils.firstN(sorted(tmp, key=itemgetter('val'), reverse=True), score)
    return rule, most
